...
use crate::render_device::{Buffer, RenderDevice, Texture, TextureView, TextureDesc};
...
impl AllocTexture2D {
    pub fn new(texture: Texture, view: TextureView) -> AllocTexture2D {
        ...
        if (height * tile_size != desc.height) || (width * tile_size != desc.width) {
            println!("Warning: texture size is not optimal. {}x{} -> {}x{}. Some memory is watsted", desc.width, desc.height, width * tile_size, height * tile_size);
        }
        
        let tree_depth = width.trailing_zeros() + 1;
        let occupation_per_layer = (0..desc.array_len).map(|_| BooleanQuadtree::new(tree_depth)).collect();
        
        AllocTexture2D { texture, view, tree_depth, occupation_per_layer }
    }
...
impl RednerLoop {
    pub fn new(rd: &RenderDevice) -> RednerLoop {
        ...
        // Create depth buffer
        let depth_buffer = rd.create_texture(TextureDesc::new_2d(
            surface_size.width,
            surface_size.height,
            vk::Format::D16_UNORM,
        ...
        .unwrap();
        let depth_buffer_view = rd.create_texture_view(&depth_buffer, depth_buffer.desc.format, vk::ImageAspectFlags::DEPTH).unwrap();
