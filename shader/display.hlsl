...
//------------------------------------
// Frostbite Neutral Display Transform 
//------------------------------------

// Code copied from the bonus slides.
// [Alex 2017 "HDR Color Grading and Dispaly in Frostbite"]
...
// From linear sRGB to non-linear sRGB. Some call this the sRGB OETF.
// https://chrisbrejon.com/cg-cinematography/chapter-1-color-management/#transfer-functions-oetf-and-eotf
// https://registry.khronos.org/DataFormat/specs/1.3/dataformat.1.3.html#TRANSFER_SRGB_INVEOTF
float srgb_eotf_inv(float v)
{
    if (v <= 0.0031308)
        return v * 12.92f;
...
float3 srgb_eotf_inv_float3(float3 col)
{
    return float3(
        srgb_eotf_inv(col.r),
        srgb_eotf_inv(col.g),
