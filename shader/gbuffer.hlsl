...
uint4 encode_gbuffer(GBuffer gbuffer) {
    ...
    uint metallic_unorm = uint(gbuffer.metallic * 255.0f);
    enc.b = roughness_unorm | metallic_unorm << 8;
...
GBuffer decode_gbuffer(uint4 enc) {
    ...
    uint metalic_unorm = (enc.b >> 8) & 0xFF;
    gbuffer.perceptual_roughness = roughness_unorm / 255.0f;
    gbuffer.metallic = metalic_unorm / 255.0f;
