...
Texture2D<float3> prev_ind_spec_texture;
Texture2D<float3> curr_ind_spec_texture;
RWTexture2D<float3> rw_filtered_ind_spec_texture;
...
void main(uint2 dispatch_id: SV_DispatchThreadID) 
{
    ...
    {
        rw_filtered_ind_spec_texture[dispatch_id] = float3(0, 0, 0);
        return;
    }

    float3 src_color = curr_ind_spec_texture[dispatch_id];
    ...
    float3 prev_color;
    {
        ...
        prev_color = prev_ind_spec_texture.SampleLevel(sampler_linear_clamp, history_uv, 0.0);
    }
    ...
    rw_filtered_ind_spec_texture[dispatch_id] = filtered_color;
}